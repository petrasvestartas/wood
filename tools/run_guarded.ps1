...
if (-not $AllowConcurrent) {
    ...
    if ($running.Count -gt 0) {
        ...
        throw ("run_guarded: {0} is already running (PID {1}). Refusing to start a second " +
               "copy - concurrent runs are what exhausted memory on 2026-08-27. Kill it " +
               "first, or pass -AllowConcurrent if you are certain." -f $procName, $ids)
    }
}
...
if ($proc.ExitCode -ne 0 -and $peak -ge ($MemoryLimitGB * 0.95)) {
    Write-Warning ("run_guarded: peak commit sat at the {0} GB cap - the memory limit is what " +
                   "stopped this run. That is a bug in the program, not in the cap." -f $MemoryLimitGB)
}
exit $proc.ExitCode
