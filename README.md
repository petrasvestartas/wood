...
| `templates_gridshell` | `src/templates/shells/lamella_gridshell.h`: asymptotic and iso-curve gridshells on a saddle surface, then asymptotic ones on three minimal meshes (a relaxed disk, a catenoid annulus, an Enneper disk); two upright `BeamCurved` boards per lamella (a section swept along a smooth central axis into one BRep: four rails, ruled faces) in two layers and a hexagonal stud at every crossing; prints per scene the minimality of the mesh, the normal curvature and unrolled straightness of the lamellas and the largest overlap, exits 1 unless every board is a valid BRep solid, every stud touches its four boards, nothing overlaps and every board loads back from the file as a `BeamCurved` |
