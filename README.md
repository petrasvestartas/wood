...
| `templates_gridshell` | `src/templates/shells/lamella_gridshell.h`: asymptotic, iso-curve and constant-normal-curvature gridshells on NURBS surfaces (a cubic saddle, a hyperbolic paraboloid, a half catenoid fitted as a rational NURBS); every lamella a curve on the surface traced in the parameter plane by RK4 as Bowerbird's Pathfinder, along the directions of normal curvature 0 by Euler's formula; boards are `BeamCurved` slabs of each lamella's rectifying developable (rulings tg t + kg n, Schling et al. 2022), held to the normal at the nodes, a hexagonal stud on the exact surface normal at every crossing; prints per scene the normal curvature, geodesic curvature and torsion, ruling lean, twist, unrolled straightness, stud fit and overlap, exits 1 unless every board is a valid BRep solid, every asymptotic lamella is straight in the normal, every stud fits at its node, nothing overlaps and every board reloads as a `BeamCurved` |
