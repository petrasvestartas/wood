...
| `templates_gridshell` | `src/templates/shells/lamella_gridshell.h`: asymptotic and iso-curve gridshells on a saddle surface, then asymptotic ones on three minimal meshes (a relaxed disk, a catenoid annulus, an Enneper disk); the traced curves become a discrete net optimised as in Wang, Almaskin, Pottmann 2025 (shared A-net node normals), two continuous twisting `BeamCurved` boards per lamella and a hexagonal stud on every node normal; prints per scene the net residual, twist, normal curvature, unrolled straightness, stud fit and overlap, exits 1 unless every board is a valid BRep solid, every stud fits at its node, nothing overlaps and every board reloads as a `BeamCurved` |
