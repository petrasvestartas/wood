...
| Lamella gridshell | `shells/lamella_gridshell.h` | `templates_gridshell` | two upright BRep boards per lamella on iso or asymptotic curves of a surface or a minimal mesh, in two layers, a hexagonal stud at every crossing |
...
A two-directional, two-layer lamella gridshell, after the asymptotic gridshells of Eike Schling, on a NURBS surface (`Gridshell::from_surface`) or on a triangle or quad mesh (`Gridshell::from_mesh`). Both go through one tracer over a `Field`: a `SurfaceField` works in (u, v), a `MeshField` in xyz on the mesh. On a surface `curves` picks the two lamella families: 0 the u and v iso-curves, on any surface; 1 the asymptotic curves, where the normal curvature II(d, d) is zero, on a surface of negative Gaussian curvature. On a mesh the families are always the asymptotic curves: every vertex gets a normal (Max's weights) and a shape operator (least squares of the turn of the normals along the edges of each face, averaged over the faces around), both are blended inside a triangle, and the directions are where the blended form vanishes; a point on the mesh is lifted off its flat triangle by half of Phong tessellation, so a traced curve lies on the smooth surface the mesh samples and not on its facets. Each family is traced with fixed RK4 steps of `step` mm, as Bowerbird does, from seeds at even arc lengths along a spine of the other family through the middle of the domain (on a mesh the vertex nearest the centroid), until it reaches the surface edge, or on a mesh the band of triangles touching the boundary, whose one-sided normals are not trusted; the crossings are found segment against segment in the local tangent plane. The first family is the top layer, `spacing / 2` up the local normal, the second the bottom layer, as far down.

Every lamella is two upright boards, `height` along the local normal and `thickness` across, `gap` apart. A board is a `BeamCurved`, the wood element for curved beams: a central axis, the cubic `NurbsCurve::create_interpolated` through the lamella's stations (shared by both boards of a lamella), the station parameters on it, an up direction per station (the local surface normal) and one closed cross-section in the station frame (x across the axis, y up; here `profile_rectangle(thickness, height)` moved up and across). The sweep places the section at every station, runs one cubic rail through each section corner, puts a ruled face (`Primitives::create_ruled`) between each two neighbouring rails and a planar cap at each end, and closes them into one BRep solid with the rails as its long edges. So a board is smooth along its whole length, straight across, and kinks only at its section's corners and its two ends; there is no face or band per segment and no loft through the sections. Contacts and the clash check read the same sweep as a mesh through the placed sections, every vertex on a rail. `BeamCurved` is written as `wood_proto.BeamCurved` (`src/proto/element_beam_curved.proto`) and registered, so a saved session loads back as curved beams. The boards run straight for a gap either side of every crossing and past each end, and follow the curve between. At every crossing a `Column` stud, a BRep prism, runs along the normal through the gaps of both layers, `overrun` past each outer face. Its section is a hexagon of three flat pairs, all `gap` across: one pair against the top boards, one against the bottom boards and one across the long corners, so the stud touches all four boards and cuts none; a 60 degree crossing gives the regular hexagon. `spacing` must be at least `height`, or the layers overlap.

Along an asymptotic curve the normal curvature is zero, so a board standing on the normal bends only about its weak axis and unrolls to a straight strip: the point of the system. On a minimal surface the two asymptotic families cross at right angles everywhere, which is why minimal meshes are the natural carriers to explore topologies with. The example builds five scenes in a row: a 10 m saddle surface in asymptotic mode and a 6 m one in iso mode, then three minimal meshes, one per topology: a 10 m disk relaxed to a minimal surface inside the saddle's boundary by cotangent Laplacian sweeps, a catenoid annulus between two 6 m rings and a 9 m Enneper disk. For each it prints how far a mesh is from minimal (the largest |k1 + k2| / sqrt(2 (k1^2 + k2^2)) off the boundary band), the largest normal curvature along the lamellas, the largest distance of an unrolled lamella from a straight line, the largest tilt of a board side's ruling from the normal halfway between two sections and the largest overlap volume between two elements. It then finds the face contacts and fails unless every board is a valid six-face BRep solid, every stud touches its four boards and no overlap is larger than the tolerance. Every board and stud is written to the file as its BRep.
