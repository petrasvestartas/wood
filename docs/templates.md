...
Every lamella is two upright boards, `height` along the local normal and `thickness` across, `gap` apart. A board is a `Board`, a `Beam` on the lamella centreline whose sections are rectangles the local normal and the normal cross the tangent span, moved up and across, and whose solid is a BRep: one cubic rail interpolated through each corner of every section (`NurbsCurve::create_interpolated`), a ruled face between each two neighbouring rails (`Primitives::create_ruled`) and a planar cap at each end, closed into one solid with the rails as its long edges. So a board is smooth along its whole length, straight across, and kinks only at its four long edges and its two ends; there is no face or band per segment and no loft through the sections. Contacts and the clash check read the same solid sampled at its sections, every vertex on a rail. The boards run straight for a gap either side of every crossing and past each end, and follow the curve between. At every crossing a `Column` stud, a BRep prism, runs along the normal through the gaps of both layers, `overrun` past each outer face. Its section is a hexagon of three flat pairs, all `gap` across: one pair against the top boards, one against the bottom boards and one across the long corners, so the stud touches all four boards and cuts none; a 60 degree crossing gives the regular hexagon. `spacing` must be at least `height`, or the layers overlap.
