...
| `src/templates/grid/grid_plan.h/.cpp` | the plan geometry of the grid template (`compute_section` of a solid at a height, ring booleans over Clipper2, offsets and mitres, `compute_pieces` of a line against rings, `compute_crossings` and `compute_arrangement` of lines into a `Mesh` arrangement, direction polygons at a node) and the records and joint rules `grid_joints.cpp` and `grid.cpp` share |
...
| `src/joinery_solver/wood_datasets.h/.cpp` | dataset runners, one per `data/*.yml` |
...
| `examples/` | `1_elements` … `12_cross_joints` (`docs/examples.md`), `main_dataset_runner`, `main_all_datasets`, `main_session_round_trip`, `main_element_mapping_check`, one `templates_*` main per template, and for the grid one per workflow (`templates_grid_{footprint,solid,lines}`), the references (`templates_grid_reference`) and the joints and sections with the clash check (`templates_grid_framing`) |
