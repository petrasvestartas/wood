...
- `compute_joints` runs `wood_feature_solver`: `adjacent_pairs` (the sidecar or the search), `wood_feature_detection` on each pair (one `FeaturePlate` or nothing, and possibly a face swap on the second plate), `wood_three_valence` (shadow joints, annen alignment), `wood_feature_construction` + the joint library (unit-box outlines, oriented onto the volumes), `wood_merge_modifier` (the cut outlines stitched into each plate's `features`), then every joint onto its interaction with `add_joint` and onto both hosts as `ElementFeature`s.
...
- **Vocabulary is split.** The records say feature, the pipeline says joint: `compute_joints`, `get_joints`, `Plate::joint_types`, `joint_type`, `JOINTS_PARAMETERS_AND_TYPES`, `SearchType::face_to_face`. One word should win.
- **Two outputs from one call.** `compute_joints` both stores the joints and returns them; callers can act on the return value while the store holds an older copy.
...
- **`FeaturePlate` holds only what is fabricated.** Links between features by guid, not by index; trace counters in a separate `DetectionTrace` returned by detection; the pair and the contact read through the interaction, unless the duplication is kept on purpose, in which case `add_joint` asserts they agree.
