...
namespace xml_parser
{
    ...
    bool read_xml_polylines(std::vector<std::vector<IK::Point_3>> &polylines, bool simple_case = false);

    bool write_xml_polylines(std::vector<std::vector<IK::Point_3>> &polylines, bool simple_case = false);
