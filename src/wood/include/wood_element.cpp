...
void element::get_joints_geometry(std::vector<joint> &joints, std::vector<std::vector<CGAL_Polyline>> &output, int what_to_expose, std::vector<std::vector<cut_type>> &output_cut_types)
{
    // you are in a loop
    // printf("/n %i", id);
    ...
    { // loop joint id
        ...
        { // loop joints per each face + 1 undefined
            ...
            {
                continue;
                if (joints[std::get<0>(j_mf[i][j])](std::get<1>(j_mf[i][j]))[k] != drill)
                    continue;
...
void element::merge_joints(std::vector<joint> &joints, std::vector<std::vector<CGAL_Polyline>> &output)
{
