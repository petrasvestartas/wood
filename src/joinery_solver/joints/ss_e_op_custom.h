// ss_e_op_custom: user-supplied unit-cube joint geometry for ss_e_op (type 11).
// Port of wood_joint_lib.cpp:2279-2303 — reads pairs (i, i+1) from
// CUSTOM_JOINTS_SS_E_OP_MALE/FEMALE. Each pair = (face0, face1) polyline for
// one outline entry.
//
// Upstream pushes each polyline twice into the same slot (producing a 4-point
// endpoint marker). Session merge requires the marker at jm[0][1] to be 2 or 5
// points (wood_merge.cpp:145-146); 4-point → merge skips entirely. So we
// follow ss_e_ip_custom's approach: concatenate all per-pair polylines into one
// outline + derive a real 2-point endpoint marker from first/last point.
