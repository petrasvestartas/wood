...
static void ts_e_p_3(WoodJoint& joint) {
    ...
    const Polyline& m0pts = joint.m_outlines[0][0];
    const Polyline& m1pts = joint.m_outlines[1][0];
    ...
    for (int f = 0; f < 2; f++) {
        if (size < 2 || joint.f_outlines[f].empty())
            continue;
        const Polyline& first = joint.f_outlines[f].front();
        const Polyline& last = joint.f_outlines[f].back();
    ...
    for (int f = 0; f < 2; f++) {
        std::vector<int> cuts;
        cuts.reserve(joint.f_outlines[f].size());
        for (size_t k = 0; k + 1 < joint.f_outlines[f].size(); k++)
            cuts.push_back(wood_cut::hole);
        if (!joint.f_outlines[f].empty())
            cuts.push_back(wood_cut::insert_between_multiple_edges);
        joint.f_cut_types[f] = std::move(cuts);
    }
