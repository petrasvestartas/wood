...
PolylineMerge::PolylineMerge(const Plate& plate, int plate_index)
...
double PolylineMerge::perpendicular_distance_squared(const Point& p, const Point& line_a, const Point& line_b) {
    const Vector direction = line_b - line_a;
    const double length_squared = direction.magnitude_squared();
    if (length_squared < 1e-20)
        return 0.0;
    const double t = (p - line_a).dot(direction) / length_squared;
    const Point projected = line_a + direction * t;
    return (p - projected).magnitude_squared();
}
...
void PolylineMerge::log_points(std::ofstream& log, const Polyline& polyline) {
    for (size_t k = 0; k < polyline.point_count(); k++) {
        const Point p = polyline.get_point(k);
        log << " (" << p[0] << "," << p[1] << "," << p[2] << ")";
    }
}

void PolylineMerge::log_plate() const {
    if (!log || plate_index < 0)
        return;
    std::ofstream& out = *log;
    out << "ELEMENT " << plate_index
        ...
        << ")\n";
    out << "  pline0 pts=" << plate.polylines[0].point_count();
    log_points(out, plate.polylines[0]);
    out << "\n  pline1 pts=" << plate.polylines[1].point_count();
    log_points(out, plate.polylines[1]);
    out << "\n";
}

void PolylineMerge::log_result(const Polyline& merged_top, const Polyline& merged_bottom) const {
    if (!log)
        return;
    std::ofstream& out = *log;
    out << "  MERGED el=" << plate_index
        ...
        << "\n";
    out << "  merged_top:";
    log_points(out, merged_top);
    out << "\n  merged_bot:";
    log_points(out, merged_bottom);
    out << "\n";
}
...
PolylineMerge::JointOutlines* PolylineMerge::joint_outlines(WoodJoint& joint, size_t face, int joint_id, bool male_or_female) const {
    JointOutlines& outlines = male_or_female ? joint.m_outlines : joint.f_outlines;
...
void PolylineMerge::insert_rectangle_cut(const JointOutlines& outlines) {
...
PolylineMerge::RelocatedCorners PolylineMerge::corner_intersections(size_t face, int previous, int next, bool z_axis_valid) const {
...
void PolylineMerge::relocate_previous_corners(size_t face, const Point& top_start, const Point& bottom_start, RelocatedCorners& corners) const {
...
bool PolylineMerge::relocate_edge_vertices(JointOutlines& outlines, size_t face) {
    ...
    if (top_points.size() < 4 || joint_planes.size() != top_points.size() + 1)
        return false;
    const size_t n = top_points.size() - 1;
    const int id = static_cast<int>(face) - 2;
    const int previous = ((int)n + id - 1) % (int)n;
...
void PolylineMerge::flip_and_insert_cut(const WoodJoint& joint, JointOutlines& outlines, size_t face, int joint_id, bool male_or_female) {
    const int id = static_cast<int>(face) - 2;
    ...
    if (reference.point_count() >= 1) {
        ...
        const Point reference_point = top_points[id + 1];
    ...
    const std::pair<double, double> parameters(id + 0.1, id + 0.9);
    const size_t key = (size_t)(EDGE_SCALE * std::floor(parameters.first)) + (size_t)(FRACTION_SCALE * std::fmod(parameters.first, 1.0));
    if (log) {
    ...
    top_runs.insert({key, {parameters, outlines[0][0].get_points()}});
    bottom_runs.insert({key, {parameters, outlines[1][0].get_points()}});
}

void PolylineMerge::insert_side_joints(const PlateMembership& membership, std::vector<WoodJoint>& joints) {
...
Polyline PolylineMerge::build_merged_outline(const std::vector<Point>& points, SortedRuns& runs, const Point& original_front) {
    std::vector<bool> point_flags(points.size(), true);
    for (const auto& kv : runs) {
        const std::pair<double, double>& parameters = kv.second.first;
    ...
    for (size_t k = 0; k < point_flags.size(); k++) {
        if (!point_flags[k])
            continue;
        const size_t key = (size_t)(k * EDGE_SCALE);
        runs.insert({key, {{(double)k, (double)k}, std::vector<Point>{points[k]}}});
    }
...
void PolylineMerge::close_corner(Polyline& merged_top, Polyline& merged_bottom) const {
...
void PolylineMerge::cut_holes_top_bottom(const PlateMembership& membership, std::vector<WoodJoint>& joints, std::vector<Polyline>& result) const {
    for (size_t face = 0; face < 2 && face < membership.size(); face++) {
        for (size_t k = 0; k < membership[face].size(); k++) {
            ...
            WoodJoint& joint = joints[joint_id];
            JointOutlines& outlines = male_or_female ? joint.m_outlines : joint.f_outlines;
            auto& cut_types = male_or_female ? joint.m_cut_types : joint.f_cut_types;
...
void PolylineMerge::cut_holes_side(const PlateMembership& membership, std::vector<WoodJoint>& joints, std::vector<Polyline>& result) const {
