...
static void linked_geometry() {
    auto joints = linked_joints({1, 1, 0, 1});
...
static void missing_datasets(const std::filesystem::path& folder) {
    const auto empty = folder / "empty.obj";
    std::ofstream(empty).close();
    for (const auto& path : {empty, folder / "missing.obj"}) {
        bool rejected = false;
        try {
...
static void dataset_tolerance(const std::filesystem::path& folder) {
    const auto path = folder / "axis.obj";
    {
        std::ofstream file(path);
        file << "v 0 0 0\nv 0.01 0 0\nv 1 0 0\ncurv 0 1 1 2 3\nend\n";
    }
    globals::DUPLICATE_PTS_TOL = 0.1;
    auto axes = internal::load_polylines(path.string());
...
int main() {
    ...
    const auto folder = std::filesystem::temp_directory_path() / ("wood-solver-" + ::guid());
