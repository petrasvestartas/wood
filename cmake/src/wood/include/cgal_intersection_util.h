 ...
 */
int solve3x3 (const double (&row0)[3], const double (&row1)[3], const double (&row2)[3], double d0, double d1, double d2, double *x_addr, double *y_addr, double *z_addr, double *pivot_ratio);
 ...
 * @return true the intersection is sucsessful all the four plane_plane_plane intersections were
 * successful
 */
 ...
 * WARNING: this method is very specific to the use case of plane_to_face mehtod in the
 * wood_main.cpp
 *
...
/**
 * Intersect plane_pair0_0 with plane_pair1_0 and plane_pair1_1 and return the orthogonal vector
 * between the two lines Two planes are given Other two are contructed from a line segment
 *
 * @param [in] plane_pair0_0 first plane
 * @param [in] plane_pair1_0 second plane
 * @param [in] plane_pair1_1 third plane
 * @param [out] output vector
 * @return true, if the two plane intersections are successful, but the line paralleism is not
 * checked
 */
...
/**
 * Get a closed quad from four planes and the main_plane intersection, the outline is open, the last
 * point is not the first point
 *
 * @param [in] main_plane
 * @param [in] sequence_of_planes C-type array of four planes
 * @param [out] output quad CGAL polyline
 ...
 * WARNING: this method is used imprecise CGAL - boost method, it is not recommended to use it, it
 * is probably causing non-planarity in the merge function
 *
 * @param [in] polyline CGAl_Polyline
 * @param [in] plane The plane that intersect the polylin
 * @param [in] alignment_segment Align intersection points according to an average polygon edge
 * direction
