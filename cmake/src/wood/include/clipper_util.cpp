    ...
    for (auto &polyline : input)
        {
            ...
            for (auto &point : polyline)
                output.back ().emplace_back (point.x / (double)wood::GLOBALS::CLIPPER_SCALE, point.y / (double)wood::GLOBALS::CLIPPER_SCALE, 0);
    ...
    if (C.size () > 0)
        {
            ...
            if (is_not_small) //* the result is not a line, not a triangle, and not a very small
                              // polygon
                {
...
bool
offset_and_divide_to_points (std::vector<IK::Point_3> &result, std::vector<IK::Point_3> &polygon, double offset_distance, double division_distance)
{
    // offset polygon
    IK::Point_3 center;
    IK::Plane_3 plane;
