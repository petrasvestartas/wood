...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
IK::Point_3
point_at (const IK::Segment_3 &line, const double &t)
{
    const double s = 1.0 - t;

    return IK::Point_3 ((line[0].x () == line[1].x ()) ? line[0].x () : s * line[0].x () + t * line[1].x (), (line[0].y () == line[1].y ()) ? line[0].y () : s * line[0].y () + t * line[1].y (),
                        (line[0].z () == line[1].z ()) ? line[0].z () : s * line[0].z () + t * line[1].z ());
}

bool
closest_point_to (const IK::Point_3 &point, const IK::Segment_3 &line, double &t)
{
    bool rc = false;
    ...
    for (auto i = 0; i < input_insertion_lines.size (); i++)
        {
            ...
            auto callback = [i, &input_insertion_lines, &input_polyline_pairs, &output_insertion_vectors, &collision_count, &p0, &p1, &v] (int foundValue) -> bool {
                size_t edge = 0;
                double closest_distance = std::abs (internal::get_closest_distance (p0, input_polyline_pairs[foundValue * 2 + 0],
                                                                                    edge)); // first polyline - one starting point
                ...
                edge = 0;
                closest_distance = std::abs (internal::get_closest_distance (p0, input_polyline_pairs[foundValue * 2 + 1],
                                                                             edge)); // second polyline - one starting point
                // CGAL_Debug(closest_distance);
