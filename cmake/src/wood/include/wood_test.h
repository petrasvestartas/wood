...
//
// HISTORY:
// 1) The first version was written during the PhD 8928 thesis of Petras
// Vestartas called: Design-to-Fabrication Workflow for Raw-Sawn-Timber
// using Joinery Solver, 2017-2021 2) The translation from C# to C++ was
// started during the funding of NCCR in two steps A - standalone C++
// version of the joinery solver and B - integration to COMPAS framework
// (Python Pybind11)
//
// RESTRICTIONS:
// The code cannot be used for commercial reasons
// If you would like to use or change the code for research or educational
// reasons, please contact the developer first
//
// PROTOCOL FOR IMPLEMENTING NEW TESTS
// STEP 1 - Example method, with Doxygen type comments in the header files
// and naming: "type_joinery_plates_name_hexbox_and_corner" STEP 2 -
// GoogleTest method STEP 3 - Screenshot , click "GLFW_KEY_KP_0" to save
// the screenshot wood::GLOBALS::cpp -> DATA_SET_INPUT_FOLDER
// (C:\IBOIS57\_Code\Software\Python\compas_wood\backend\src\wood\dataset)
// STEP 4 - Run all the tests in the main.cpp like this:
// wood::test::run_all_tests(); NOTE - file-paths that are different from
// pc to pc, please change the path before running the code in
// wood::GLOBALS::cpp -> DATA_SET_INPUT_FOLDER
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 * set properties for saving the screenshot files e.g.
 * "C:\IBOIS57\_Code\Software\Python\compas_wood\backend\src\wood\dataset"
 */
...
/**
 * example for plates
 * hex-box from the Rossiniere project development and a simple rectangle
 * corner seen in Annen project
 */
bool type_plates_name_hexbox_and_corner ();

/**
 * example for plates
 * 4 types of connections for a) simple tenon mortise, b) four linked
 * joints, c) two linked and one not linked joint but touching the wall,
 * existing in vidy vertical walls only, d) two linked and one not linked
 * joint this is running clipper method with rectangles for the special
 * joint
 */
...
/**
 * example for plates
 * two layer st loup chapel in one axis, since the rest of the axis have
 * the same geometrical joinery generation, just angles are different this
 * is running clipper method with rectangles for the special joint
 */
 ...
 * be aware that the insertion direction is the normal of the
 * joint-volume-rectangle, in other words, it is the insertion direction
...
/**
 * example for plates
 * out_of_plane finger joints, ending with miters at the corner in a form
 * of a box
 */
...
/**
 * example for plates
 * in_plane and out_of_plane finger joints, simple two plates corners that
 * shows different rotations between plates
 */
...
/**
 * example for plates
 * example for developing screw connection when plates are touching
 * top-to-top
 */
...
/**
 * example for plates
 * in_plane and out_of_plane finger joints and top_to_top joints in one
 * assembly - Rossiniere First Version from timber boards look how simply
 * we can change the planarity tolerance: wood::GLOBALS::DISTANCE_SQUARED
 * *= 100; THERE IS A BUG IN THE VIEWER, DISPLAY DISAPPEARS AFTER GOING
 * FROM 4 TO 3 TO 4 MODE
 */
 ...
 * top-to-side and side-to-side out-of-plane joints
 * Annen project
 * two boxes with insertion joints and joint type selection and the
 * 3-valence joint
 */
 ...
 * Annen project
 * two boxes with insertion joints and joint type selection and the
 * 3-valence joint a small version of the full arch is given - special xml
 * function is needed
 */
 ...
 * Annen project
 * two boxes with insertion joints and joint type selection and the
 * 3-valence joint full arch is given - special xml function is needed
 */
...
/**
 * example for plates
 * cross joints, hexagonal meshes edges rotated by normal to create a
...
/**
 * example for plates
 * cross joints, single layer hexagon shell connected with connectors in a
 * shape of an arch
 */
...
/**
 * example for plates
 * cross joints, vda pavilion consisting of two layers quad panels and
 * connectors
 */
...
/**
 * example for plates
 * cross joints, square reciprocal structure taken from an architecture
 * museum in reality joints are connected by metal holders
 */
...
/**
 * example for beams
 * multiple beams connected in one node, taken from the Phanome project
...
/**
 * example for geometry
 * create minimum bounding box for a set of points using convex hull and
 * cull them using a polygon
 */
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 * googletest method that must be run in the main.cpp to check all the
 * functions above
 */
void run_all_tests ();
